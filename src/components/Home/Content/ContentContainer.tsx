...
const ContentContainer:React.FC<IContentContainer> = ({ subTitle, title, text, img, link = '#', imageRight, hideBubble}) => {
    return (
        <div className={`px-8 flex flex-col items-center justify-between ${imageRight ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
            ...
            <div className="md:w-6/12 relative w-full mt-10 md:mt-0">
                ...
                />
                <div className={`z-0 absolute left-0 top-0 img-morph bg-primary-purple_light w-[250px] h-[250px] md:w-[350px] md:h-[350px] ${hideBubble ? 'hidden' : ''}`}></div>
            </div>
        </div>
    );
};
